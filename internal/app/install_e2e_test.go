...
func TestLaunchdInstallSyncRestartAndUninstall(t *testing.T) {
	...
	opts := setupOptions{skill: testSkillBundle("launchd fixture"), configPath: configPath, binary: binary, in: strings.NewReader("\nyes\n"), runner: execCommandRunner{}, service: service}
	var out strings.Builder
	opts.out = &out
	if err := runSetup(context.Background(), opts); err != nil {
		t.Fatalf("install: %v\n%s", err, out.String())
	}
	if got := skillRead(t, filepath.Join(home, ".agents", "skills", "repo-sync", "SKILL.md")); got != "launchd fixture" {
		t.Fatalf("setup did not install offered skill: %s", got)
	}
	...
	for _, path := range []string{binary, skillRecordPath(), filepath.Join(home, ".agents", "skills", "repo-sync"), configPath, statusPath(configPath), service.plistPath(home), updaterService(service).plistPath(home), filepath.Join(home, "Library", "Logs", "repo-sync")} {
